  ...
  <div>
    <el-row>
      <el-col :span="16" :offset="4">
        ...
        >
          ...
          <el-table-column label="链接" prop="url" sortable="custom" align="center">
            <template slot-scope="{row}">
              <template v-if="row.id === editId">
                <el-input v-model="editTableForm.url" class="edit-input" size="small" clearable />
              </template>
    ...
    >
      <el-form ref="addForm" :rules="rules" :model="addDialogForm" label-width="80px" size="mini" class="dialogForm">
        ...
        <el-form-item label="来源链接" :label-width="formLabelWidth" prop="url">
          <el-input v-model="addDialogForm.url" clearable />
        </el-form-item>
...
<script>
import { fetchSourceList, deleteSourceById, updateSourceById, addSource } from '@/api/source'
export default {
  name: 'SourceList',
  data () {
    return {
  ...
  methods: {
    submitEditSource () {
      updateSourceById(this.editTableForm)
        .then((res) => {
          if (res.success) {
            const index = this.sourceList.findIndex((item) => item.id === this.editTableForm.id)
            this.sourceList.splice(index, 1, this.editTableForm)
            this.$message({
              message: '更新成功',
              type: 'success'
            })
    ...
    deleteSource(attrId) {
      ...
      deleteSourceById(params)
        .then((res) => {
          if (res.success) {
            const index = this.sourceList.findIndex((item) => item.id === attrId)
    ...
    submitAdd () {
      this.$refs['addForm'].validate((valid) => {
        if (valid) {
          addSource(this.addDialogForm)
            .then((res) => {
              if (res.results) {
                this.sourceList.push(res.results)
                this.$message({
                  message: '添加成功',
                  type: 'success'
                })
