...
<script>
import { fetchUserRoleList, updateRoleById, addRole, deleteRoleById } from '@/api/user'
...
export default {
  ...
  methods: {
    checkPermission,
    submitEditSource () {
      updateRoleById(this.editTableForm)
        .then((res) => {
          if (res.success) {
            const index = this.roleList.findIndex((item) => item.id === this.editTableForm.id)
            this.roleList.splice(index, 1, this.editTableForm)
            this.$message({
              message: '更新成功',
              type: 'success'
            })
    ...
    deleteRole(attrId) {
      ...
      deleteRoleById(params)
        .then((res) => {
          if (res.success) {
            const index = this.roleList.findIndex((item) => item.id === attrId)
